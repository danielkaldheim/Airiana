        ...
        for each in msg:
            if each.find("HWaddr") == -1 or each.find("ether") != -1:
                ...
                try:
                    ...
                    if sizeT == 5 * 1024 and "keep-log" not in sys.argv:
...
def update_sensors():
    ...
    try:
        ...
        try:
            ...
            pass  # device.msg +="\error on sensor 91"
        try:
            ...
            pass  # device.msg +="\error on sensor 92"
...
def logger():
    ...
    if "homeAss" in sys.argv:
        os.system("./ha-httpsensor.py -n Indoor -u °C -d temperature -v " + str(
            round(device.extract_ave, 1)) + ">/dev/null &")
        os.system(
            "./ha-httpsensor.py -n Outside -u °C -d temperature -v " + str(round(device.inlet_ave, 1)) + ">/dev/null &")
        try:
            os.system("./ha-httpsensor.py -n Efficiency -u % -d calculated -v " + str(
                round(numpy.average(device.eff_ave), 2)) + ">/dev/null &")
        except:
            pass
        os.system(
            "./ha-httpsensor.py -n Humidity -d humidity -u % -v " + str(int(device.new_humidity)) + ">/dev/null &")
...
def set_monitoring(monitor_state):
    global monitoring
...
class Systemair(object):
    ...
    def print_attributes(self):
        for each in dir(self):
            ...
            if isinstance(obj, (int, float, str, list)):
    ...
    def update_temps(self):
        if not savecair:
            ...
            if len(self.time) > self.averagelimit:
                self.time.pop(-1)
            ...
            if self.system_name == "VR400":
                ...
                if self.sf != 0:
                    self.tcomp = ((req.response[1] - req.response[4]) * self.coef) - self.dyn_coef
                    # float(7*34)/self.sf # compensation (heat transfer from duct) + (supply flow component)
                else:
                    self.tcomp = 0
                if self.rotor_active == "No" and self.inlet_coef < 0.14:
                    self.inlet_coef += 0.0001  # OFF
                if self.rotor_active == "Yes" and self.inlet_coef > 0.07:
                    self.inlet_coef -= 0.0001  # ON
            ...
            for each in [self.inlet, self.supply, self.extract, self.exhaust]:
                if len(each) > self.averagelimit:
                    each.pop(-1)
        else:
            self.time.insert(0, time.time())
            req.modbusregister(12543, 1)  # extract
            ...
            try:  # replace erronous data input when temp diff exceeds 1C between samples
                ...
                pass
            req.modbusregister(12102, 1)   # supply
            supply = req.response
            req.modbusregister(12101, 1)   # inlet
            inlet = req.response
            ...
            try:
                ...
                self.tcomp = (extract - inlet) * self.dyn_coef
                # float(7*34)/self.sf # compensation (heat transfer from duct) + (supply flow component)
            except ZeroDivisionError:
                pass
            self.extract.insert(0, float(extract + self.tcomp))
            self.supply.insert(0, float(supply))  # SUPPLY
            ...
            try:
                ...
                pass
            self.inlet.insert(0, float(inlet - tweak))  # INLET

            if len(self.extract) > self.averagelimit:
                self.extract.pop(-1)
            if len(self.exhaust) > self.averagelimit:
                self.exhaust.pop(-1)
            if len(self.supply) > self.averagelimit:
                self.supply.pop(-1)
            if len(self.inlet) > self.averagelimit:
                self.inlet.pop(-1)
            if len(self.time) > self.averagelimit:
                self.time.pop(-1)
            try:
            ...
            if len(self.eff_ave) > self.averagelimit:
                self.eff_ave.pop(-1)
    ...
    def set_fanspeed(self, target):
        ...
        if self.get_fanspeed() != target:
            os.write(ferr, "Incorrectly set fanspeed " + str(self.get_fanspeed()) + " to " + str(target) + " " + str(
                time.ctime()) + "\n")
        self.update_airflow()
    ...
    def update_xchanger(self):
        ...
        if self.fanspeed != 0:
            ...
            if self.rotor_active == "Yes":
                ...
                factor = 1
            if self.rotor_active == "Yes":
                self.supply_power = self.used_energy - 0 - (self.extract_ave - self.inlet_ave) * factor
            ...
            # float(8)*(self.extract_ave-self.supply_ave)# + 20Watt/degC transfer after exchanger unit
            ...
            try:
                self.loss = self.airdata_inst.energy_flow(self.ef, self.exhaust_ave, self.inlet_ave) +\
                    self.airdata_inst.energy_flow(self.sf,
                                                  self.extract_ave,
                                                  self.supply_ave)
            except:
                self.loss = 0
            try:
                ...
                pass
            if len(self.diff_ave) > self.averagelimit:
                self.diff_ave.pop(-1)
            ...
            if len(self.cond_data) > self.averagelimit + 5000:
        ...
        try:
            if self.supply_power and self.ef:
                if self.supply_ave > self.inlet_ave:
                    exhaust = self.extract_ave - self.airdata_inst.temp_diff(self.supply_power, self.extract_ave,
                                                                             self.ef)
                else:
    ...
    def moisture_calcs(self, temp="None"):  # calculate moisure/humidities
        ...
        if temp is not "None":
            ...
            low_pw = self.airdata_inst.sat_vapor_press(temp)
    ...
    def derivatives(self):
        ...
        if len(self.extract) > self.averagelimit - 1:
            ...
            self.extract_dt /= (self.time[0] - self.time[self.averagelimit - 1]) / 60
    ...
    def shower_detect(self):
        ...
        if self.RH_valid == 1 and not self.shower:  # Shower humidity sensor control
            ...
            if self.ef > 50:
                lim = 0.10
            if self.extract_dt > lim + float(self.det_limit) / 100 and self.inhibit == 0 and numpy.average(
                    self.extract_dt_list) * 60 > 1.60:
                self.msg = "Shower mode engaged\n"
                if not self.shower:
                    self.shower = True
                    self.initial_temp = self.extract_ave
        ...
        if numpy.average(self.extract_dt_list) * 60 < 0.25 \
            ...
            state = False
            self.det_limit += 1
            if not self.RH_valid and self.extract_ave <= (
                    self.initial_temp + 0.3) or self.shower_initial - time.time() < -45 * 60:
                state = True
    ...
    def print_xchanger(self):
        ...
        try:
            ...
            if "debug" in sys.argv:
                tmp += "Tcomp:" + str(self.tcomp) + " at T1:" + str(self.rawdata[0][0]) + " coef:" + str(
                    round(self.coef, 4)) + " inlet coef:" + str(self.inlet_coef) + " dyn:" + str(self.dyn_coef) + "\n"
                if not savecair:
                    tmp += "Extract:" + str(self.rawdata[0][2]) + "\tInlet:" + str(
                        self.rawdata[0][1]) + "\tExhaust:" + str(self.temps[2]) + "\tSupply,pre:" + str(
                        self.temps[0]) + "\tSupply,post:" + str(self.temps[3]) + "\n"
                else:
                    tmp += "Extract:" + str(self.rawdata[0][2]) + "\tInlet:" + str(
                        self.rawdata[0][0]) + "\tSupply,post:" + str(self.rawdata[0][1]) + "\n"
        except:
            pass
        if not savecair:
        ...
        if "humidity" in sys.argv:
            ...
            if self.RH_valid:
                tmp += "Relative humidity: " + str(round(self.new_humidity, 2)) + "% Dewpoint: " + str(
                    round(self.airdata_inst.dew_point(self.new_humidity, self.extract_ave), 2)) + "C\n"
        ...
        if "debug" in sys.argv:
            ...
            tmp += "diff. humidity partial pressure in-out: " + str(
                1000 * (
                    self.airdata_inst.sat_vapor_press(self.airdata_inst.dew_point(self.new_humidity, self.extract_ave))
                    - self.airdata_inst.sat_vapor_press(
        ...
        tmp += "Loss Total: " + str(round(self.totalenergy / 1000, 3)) + "kWh Average:" + str(round(
            self.totalenergy / ((time.time() - starttime) / 3600), 1)) + "W\n"
        ...
        if self.forcast[1] != -1:
            tmp += "Weather forecast for tomorrow is: " + str(self.forcast[0]) + "C " + \
                self.weather_types[self.forcast[1]] + ".\n\n"
        ...
        if self.press_inhibit > 0:
            tmp += "Pressure change inhibited (" + count_down(self.press_inhibit, 1800) + ")\n"
        if self.modetoken >= 1:
            tmp += "Exchanger mode change inhibited (" + count_down(self.modetoken, 3600) + ")\n"
        if self.cool_mode:
            ...
            try:
                ...
                for each in range(1, 84):
                    ...
                    for address in range(100, 1100, 100):
                        try:
                            pos = address + each
                            tmp += str(address + each) + ":" + str(self.register[str(pos)]) + " " + str(
                                hex(self.register[str(pos)])) + "\t"
                        except KeyError:
                            tmp += str(pos) + ":\t\t"
                    print tmp
                if "daemon" not in sys.argv:
                    raw_input("enter to resume")
                print "break"
            except:
                traceback.print_exc()
    ...
    def dynamic_pressure_control(self):
        if not self.shower:
            ...
            if not self.cool_mode:
                if self.inlet_ave > self.indoor_dewpoint + 0.2 and self.sf != self.ef and not self.press_inhibit and not \
                        self.forcast[1] == -1:
                    self.set_differential(0)
                    if "debug" in sys.argv:
                        self.msg += "\nPressure diff to 0%"
                if (self.inlet_ave < self.indoor_dewpoint - 0.1
                    and self.sf == self.ef
    ...
    def forcast_cooling(self):
        try:
            if self.forcast[0] > 16 and int(
                    os.popen("./forcast.py tomorrows-low").read().split(" ")[0]) > self.house_heat_limit \
            ...
            os.write(ferr, "Forcast cooling error " + str(time.ctime()) + "\n")
    ...
    def cold_recovery(self):
        ...
        if self.cool_mode and not self.inhibit and not self.shower:
            ...
            if self.fanspeed == 1 and self.extract_ave > 21 and self.extract_ave + 0.1 > self.inlet_ave:
                ...
                os.write(ferr,
                         "Cooling returned to high, indoor is hotter than outside. " + str(
                             time.ctime()) + " " + str(
                             self.fanspeed) + " " + str(self.extract_ave) + " " + str(self.inlet_ave) + "\n")
            ...
            if self.inlet_ave + 0.1 > self.extract_ave and self.fanspeed != 1 and self.extract_ave > 21:
                ...
                os.write(ferr, "Cooling will wait, will try to recycle cold air by low fanspeed " + str(
                    time.ctime()) + "\n")
    ...
    def dynamic_fanspeed(self):
        ...
        if self.fanspeed != 3 \
            ...
            os.write(ferr, "Dynamic fanspeed 3 target+1.2C or dt long > 0.7C/h (" + str(
    ...
    def get_forcast(self):
        ...
        try:
            ...
            if os.stat("./RAM/forecast.xml").st_ctime < time.time() - 3600 * 24:
    ...
    def get_local(self):
        ...
        if self.prev_static_temp >= temp:
            self.local_humidity = self.moisture_calcs(
                temp - self.kinetic_compensation)  # if 24hr low is higher than current temp
        else:
            self.local_humidity = self.moisture_calcs(
        ...
        if time.localtime().tm_hour == sun and time.localtime().tm_min < 5 or self.prev_static_temp == 8:
            ...
            if self.forcast[1] != -1:
                try:
                    ...
                    if wthr_type == 15 or wthr_type == 9 or wthr_type == 10:  # zero if fog etc.
                        self.kinetic_compensation = 0
                except:
...
if __name__ == "__main__":
    ...
    try:
        ...
        while Running:  # mainloop do each pass ###########
            ...
            if device.iter % 563 == 0:
                ...
                if "debug" in sys.argv:
                    os.system("nice ./grapher.py debug & >>/dev/null")
                ...
                if "temperatur.nu" in sys.argv:
                    os.system("wget -q -O temperatur.nu"
    ...
    5: show/update values		 0: cycle winter/summer mode
            ...
            data = -1
            try:
                if cmd_socket in select.select([sys.stdin, cmd_socket], [], [], timeout)[0]:
                    try:
                ...
                if "daemon" not in sys.argv and sys.stdin in cmd_input:
                    try:
                ...
                if data != -1:
                    ...
                    if data == 7:
                        try:
                            if "daemon" not in sys.argv:
                                inp = int(raw_input("set differential pressure(-20% -> 20%):"))
                            else:
